...
import os
PE_folder = os.path.join('static','ppol')
print(PE_folder)
from werkzeug import secure_filename
app = Flask(__name__)
app.config['UPLOAD_FOLDER'] = PE_folder
...
def upload_fil():
    if request.method == 'POST':
        ...
        newfolder = os.path.join(app.config['UPLOAD_FOLDER'],f.filename)
        print(newfolder)
      
        aa = newfolder
        
        #print(secure_filename(newfolder))
        f.save(newfolder)
        bb = img_cap1.generate_caption(img_cap1.model,aa,img_cap1.wordtoint,max_tokens=30)
        language = 'en'
        myobj = gTTS(text=bb[:-4], lang=language, slow=False) 
        myobj.save("/static/speech.mp3") 
        #return 'file uploaded successfully'
        return jsonify(caption =bb)
#         return redirect("up2.html",cap=bb)
